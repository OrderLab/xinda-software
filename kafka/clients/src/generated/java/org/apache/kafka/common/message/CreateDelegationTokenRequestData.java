...
public class CreateDelegationTokenRequestData implements ApiMessage {
    ...
    public static final Schema SCHEMA_3 =
        new Schema(
            new Field("owner_principal_type", Type.COMPACT_NULLABLE_STRING, "The principal type of the owner of the token. If it's null it defaults to the token request principal."),
            new Field("owner_principal_name", Type.COMPACT_NULLABLE_STRING, "The principal name of the owner of the token. If it's null it defaults to the token request principal."),
            new Field("renewers", new CompactArrayOf(CreatableRenewers.SCHEMA_2), "A list of those who are allowed to renew this token before it expires."),
            new Field("max_lifetime_ms", Type.INT64, "The maximum lifetime of the token in milliseconds, or -1 to use the server side default."),
            TaggedFieldsSection.of(
            )
    ...
    public void read(Readable _readable, short _version) {
        if (_version >= 3) {
            ...
            if (length < 0) {
                this.ownerPrincipalType = null;
            } else if (length > 0x7fff) {
                throw new RuntimeException("string field ownerPrincipalType had invalid length " + length);
            } else {
                this.ownerPrincipalType = _readable.readString(length);
            }
        } else {
            this.ownerPrincipalType = "";
        }
        if (_version >= 3) {
            ...
            length = _readable.readUnsignedVarint() - 1;
            if (length < 0) {
                this.ownerPrincipalName = null;
            } else if (length > 0x7fff) {
                throw new RuntimeException("string field ownerPrincipalName had invalid length " + length);
            } else {
                this.ownerPrincipalName = _readable.readString(length);
            }
        } else {
            this.ownerPrincipalName = "";
        }
    ...
    public void write(Writable _writable, ObjectSerializationCache _cache, short _version) {
        ...
        if (_version >= 3) {
            if (ownerPrincipalType == null) {
                ...
                byte[] _stringBytes = _cache.getSerializedValue(ownerPrincipalType);
                _writable.writeUnsignedVarint(_stringBytes.length + 1);
                _writable.writeByteArray(_stringBytes);
            }
        } else {
            if (this.ownerPrincipalType == null || !this.ownerPrincipalType.equals("")) {
                throw new UnsupportedVersionException("Attempted to write a non-default ownerPrincipalType at version " + _version);
            }
        }
        if (_version >= 3) {
            if (ownerPrincipalName == null) {
                _writable.writeUnsignedVarint(0);
            } else {
                byte[] _stringBytes = _cache.getSerializedValue(ownerPrincipalName);
                _writable.writeUnsignedVarint(_stringBytes.length + 1);
                _writable.writeByteArray(_stringBytes);
            }
        } else {
            if (this.ownerPrincipalName == null || !this.ownerPrincipalName.equals("")) {
                throw new UnsupportedVersionException("Attempted to write a non-default ownerPrincipalName at version " + _version);
            }
        }
        if (_version >= 2) {
    ...
    public void addSize(MessageSizeAccumulator _size, ObjectSerializationCache _cache, short _version) {
        ...
        if (_version >= 3) {
            if (ownerPrincipalType == null) {
                ...
                byte[] _stringBytes = ownerPrincipalType.getBytes(StandardCharsets.UTF_8);
                if (_stringBytes.length > 0x7fff) {
                    throw new RuntimeException("'ownerPrincipalType' field is too long to be serialized");
                }
                _cache.cacheSerializedValue(ownerPrincipalType, _stringBytes);
        ...
        if (_version >= 3) {
            if (ownerPrincipalName == null) {
                ...
                byte[] _stringBytes = ownerPrincipalName.getBytes(StandardCharsets.UTF_8);
                if (_stringBytes.length > 0x7fff) {
                    throw new RuntimeException("'ownerPrincipalName' field is too long to be serialized");
                }
                _cache.cacheSerializedValue(ownerPrincipalName, _stringBytes);
                _size.addBytes(_stringBytes.length + ByteUtils.sizeOfUnsignedVarint(_stringBytes.length + 1));
            }
        }
        {
    ...
    public boolean equals(Object obj) {
        ...
        if (this.ownerPrincipalType == null) {
            if (other.ownerPrincipalType != null) return false;
        } else {
            if (!this.ownerPrincipalType.equals(other.ownerPrincipalType)) return false;
        }
        if (this.ownerPrincipalName == null) {
            if (other.ownerPrincipalName != null) return false;
        } else {
            if (!this.ownerPrincipalName.equals(other.ownerPrincipalName)) return false;
        }
    ...
    public int hashCode() {
        int hashCode = 0;
        hashCode = 31 * hashCode + (ownerPrincipalType == null ? 0 : ownerPrincipalType.hashCode());
        hashCode = 31 * hashCode + (ownerPrincipalName == null ? 0 : ownerPrincipalName.hashCode());
    ...
    public CreateDelegationTokenRequestData duplicate() {
        ...
        if (ownerPrincipalType == null) {
            _duplicate.ownerPrincipalType = null;
        } else {
            _duplicate.ownerPrincipalType = ownerPrincipalType;
        }
        if (ownerPrincipalName == null) {
            _duplicate.ownerPrincipalName = null;
        } else {
            _duplicate.ownerPrincipalName = ownerPrincipalName;
        }
    ...
    public String toString() {
        return "CreateDelegationTokenRequestData("
            + "ownerPrincipalType=" + ((ownerPrincipalType == null) ? "null" : "'" + ownerPrincipalType.toString() + "'")
            + ", ownerPrincipalName=" + ((ownerPrincipalName == null) ? "null" : "'" + ownerPrincipalName.toString() + "'")
    ...
    public CreateDelegationTokenRequestData setOwnerPrincipalType(String v) {
        this.ownerPrincipalType = v;
        return this;
    }
    
    public CreateDelegationTokenRequestData setOwnerPrincipalName(String v) {
        this.ownerPrincipalName = v;
        return this;
    }
